...
def _install_wheelhouse(work_dir) -> dict:
    ...
    # wheelhouse provides already imports, the wheels are in — skip pip entirely.
    # The probe set is DERIVED from the bundled wheels, so an --ast, --pptx, or
    # combined build each skip correctly. An unrecognised wheel (no mapping) or a
    # broken half-install falls through to the self-healing pip flow below.
    _IMPORT_NAME = {
