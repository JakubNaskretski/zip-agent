...
def ingest_jira(lib, dump_dir, author, rationale, progress=None, *, dg=None):
    ...
    ``dg`` (keyword-only): pass a pre-parsed :class:`JiraDigest` from a
    preceding ``parse_jira()`` call to skip the re-parse. ``dump_dir`` is still
