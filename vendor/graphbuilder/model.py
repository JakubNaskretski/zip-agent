...
NODE_TYPES = {
    ...
    # + extractors/docx + extractors/xlsx + extractors/pptx). Names/labels/headers
    # ONLY — cell values, formula bodies, numeric data values and author names never
    # enter the graph; `docsection` body text and `slide` text/notes are the
    # deliberate content captures (like Confluence page bodies). Structure detection
    ...
    # are a `columns` ATTR on sheet/table/section/slide nodes (a node per column is
    # noise). Reuses `contains` / `child-of` — no new edge types.
    # `slide`: one per PowerPoint slide (title, body text, notes, columns from the
    # first table). `chart`: one per embedded chart (series names + category labels;
