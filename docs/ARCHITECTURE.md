...
| 2 | **Bootstrap + session loop** — ✅ **implemented**: [`librarian/bootstrap.py`](../librarian/bootstrap.py) (`boot()` + `Session` with auto-checkpoint), [`profiles/_base/MASTER_PROMPT.md`](../profiles/_base/MASTER_PROMPT.md) (the operating contract — assembled per profile and pasted into the builder, *outside* the ZIP), [`scripts/build_memory.py`](../scripts/build_memory.py) deployable-ZIP builder. Verified by a clean-room boot: a 20 KB `memory.zip` imports the engine from *inside* itself and runs a commit cycle on an interpreter with no `librarian` installed. | Proves the ZIP-in/ZIP-out memory cycle end to end. |
...
| 4 | **Scraper + export contract + digest: Jira/Confluence** — ✅ **implemented**: read-only collectors ship inside the vendored engine ([`vendor/graphbuilder/jira/collect.py`](../vendor/graphbuilder/jira/collect.py), [`confluence/collect.py`](../vendor/graphbuilder/confluence/collect.py) — GET-only, PAT via env var, run on the user's machine; MASTER_PROMPT §7), and the digest adapters ([`librarian/digest/jira.py`](../librarian/digest/jira.py), [`librarian/digest/confluence.py`](../librarian/digest/confluence.py)) turn the dumps into one raw KU per issue/page (body = the dump JSON verbatim) + a contained per-source graph KU serialized with inline text redacted (bodies live only in the raw KUs). `entities` carry structured ids only (issue key / space key + page id) — prose is reached via FTS, never the entity bridge; no cross-source join edges. `normalize_pl.py` + `pl_lemmas.sqlite` (Polish lemmatization) stay deferred. | The hard retrieval source; the escape hatch (§7.4) de-risks it. |
